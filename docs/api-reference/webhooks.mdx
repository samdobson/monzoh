...
Monzoh provides built-in utilities to parse and validate incoming webhook payloads with proper signature verification and type safety.
...
```python
from monzoh.webhooks import parse_webhook_payload, WebhookParseError, WebhookSignatureError
...
- `webhook_secret`: Webhook secret for signature verification (required if verify_signature=True)
- `verify_signature`: Whether to verify the webhook signature (default: True)
...
async def handle_monzo_webhook(request: Request):
    ...
    try:
        # Parse webhook payload without signature verification
...
#### Secure Usage With Signature Verification
...
@app.post("/webhooks/monzo")
async def handle_monzo_webhook(request: Request):
    """Handle incoming Monzo webhook events with signature verification."""
    ...
    try:
        # Parse transaction webhook with full security validation
        ...
        # Transaction is fully validated and type-safe
...
async def process_transaction(transaction):
    """Process validated transaction with full type safety."""
    
    amount_pounds = abs(transaction.amount) / 100
    balance_pounds = transaction.account_balance / 100
    
    print(f"💳 New Transaction: {transaction.description}")
    print(f"   Amount: £{amount_pounds:.2f}")
    print(f"   Balance: £{balance_pounds:.2f}")
    print(f"   Category: {transaction.category}")
    print(f"   Merchant: {transaction.merchant}")
    
    # Business logic with validated data
    if amount_pounds > 100:
        await send_large_transaction_alert(transaction)
    
    if transaction.category == "eating_out" and "coffee" in transaction.description.lower():
        await track_coffee_spending(transaction)
```

### Setting up a Complete Webhook Server

Here's a complete example using the new webhook utilities:
...
Monzoh includes built-in signature verification that handles all security concerns:

```python
from monzoh.webhooks import verify_webhook_signature, WebhookSignatureError
...
# Automatic verification in parse functions
try:
    payload = parse_webhook_payload(
        body=body,
        headers=headers,
...
### Legacy Manual Signature Verification

If you need to implement signature verification manually:

```python
import hmac
import hashlib

def verify_webhook_signature_manual(body: bytes, signature: str, secret: str) -> bool:
