...
class Balance(BaseModel):
    balance: int                                    # Current balance in minor currency units (pence)
    total_balance: int                              # Total balance including pending transactions
    currency: str                                   # ISO 4217 currency code
    local_currency: str                            # Currency for local display
    spend_today: int                               # Amount spent today in minor currency units
...
class Transaction(BaseModel):
    ...
    amount: int                      # Amount in minor currency units (negative for spending)
    currency: str                    # ISO 4217 currency code
...
class Pot(BaseModel):
    ...
    balance: int                     # Current balance in minor currency units
    currency: str                    # ISO 4217 currency code
    goal_amount: Optional[int]       # Goal amount in minor currency units
    type: str                        # Pot type
...
class Receipt(BaseModel):
    ...
    external_id: str                 # External identifier
    total: int                       # Total amount in minor currency units
    currency: str                    # ISO 4217 currency code
...
class ReceiptItem(BaseModel):
    ...
    unit_price: int                  # Unit price in minor currency units
    quantity: float                  # Quantity
    total_price: int                # Total price in minor currency units
    currency: str                    # ISO 4217 currency code
...
class ReceiptTax(BaseModel):
    ...
    amount: int                      # Tax amount in minor currency units
    currency: str                    # ISO 4217 currency code
...
print(f"Balance: £{balance.balance / 100:.2f}")  # ✅ Type: int
```
...
def analyze_transactions(transactions: list[Transaction]) -> dict:
    ...
    total_spent = sum(abs(t.amount) for t in spending) / 100
    total_income = sum(t.amount for t in income) / 100
    ...
    return {
        ...
        "categories": {
            cat: {
                "count": len(txns),
                "total": sum(abs(t.amount) for t in txns) / 100
            }
