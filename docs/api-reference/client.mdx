...
| `retry_config` | `Optional[RetryConfig]` | `None` | Retry configuration for handling transient errors |
...
The client includes built-in retry functionality for handling transient errors. You can customize retry behavior using the `retry_config` parameter:
...
client = MonzoClient(retry_config=no_retry)
```

The retry system automatically handles:
- Rate limit errors (429) with respect for Retry-After headers
- Server errors (500)
- Timeout errors (504)
- Network connection issues

For complete details, see the [Retry Handling](/guides/retry-handling) guide.
